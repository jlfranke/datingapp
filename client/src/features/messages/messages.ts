...
export class Messages implements OnInit {
  ...
  loadMessages() {
    this.messageService.getMessages(this.container, this.pageNumber, this.pageSize).subscribe({
      next: (response) => {
        ...
        this.fetchedContainer = this.container; // Update fetchedContainer after successful load
      },
      error: (error) => {
  ...
  deleteMessage(event: Event, id: string) {
    ...
    this.messageService.deleteMessage(id).subscribe({
      next: () => {
        // Remove the deleted message from the paginatedMessages signal
        const currentMessages = this.paginatedMessages();
        if (currentMessages && currentMessages.items) {
